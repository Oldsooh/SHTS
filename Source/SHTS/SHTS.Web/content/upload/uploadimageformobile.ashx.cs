...
namespace Witbird.SHTS.Web.Content.upload
{
    ...
    /// uploadimage 的摘要说明
    /// </summary>
    public class uploadimageformobile : IHttpHandler
    {
        int width = 100;
        int height = 135;
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                else if (context.Request.TotalBytes>0)
                {
                    //保存文件的物理路径
                    string oldFile = uploadPath + fileName + ".jpeg";
                    ...
                    //保存缩略图的物理路径
                    small = folderName + timePath + fileName + "_small.jpeg";
                    string thumbnail = uploadPath + fileName + "_small.jpeg";
                    MakeThumbnail(oldFile, thumbnail, width, height, mode);

                    big = folderName + timePath + fileName + "_big.jpeg";
                    thumbnail = uploadPath + fileName + "_big.jpeg";
                    MakeThumbnail(oldFile, thumbnail, 469, 279, mode);

                    originalImage = folderName + timePath + fileName + ".jpeg";
                }
                else
                {
                    message = "请选择文件";
            ...
            catch (Exception e)
            {
                action = false;
                message = e.Message;
                LogService.Log("上传失败" + e.Message, e.ToString());
            }
