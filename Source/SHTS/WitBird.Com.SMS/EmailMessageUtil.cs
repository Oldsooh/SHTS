...
namespace WitBird.Com.SMS
{
    public class EmailMessageUtil
    {
        ...
        private string mSenderUser;//发件箱的用户名,例如：hello@163.com
        ...
        public EmailMessageUtil(string server, string fromMail, string username, string password, int port, bool sslEnable, bool pwdCheckEnable)
        {
            this.mSenderServerHost = server;
            this.mSenderUser = fromMail;
            this.mSenderUsername = username;
            this.mSenderPassword = password;
            this.mSenderPort = port;
        ...
        ///<param name="sslEnable">true表示对邮件内容进行socket层加密传输，false表示不加密</param>
        ///<param name="pwdCheckEnable">true表示对发件人邮箱进行密码验证，false表示不对发件人邮箱进行密码验证</param>
        public MailMessage CreateMailMessage(string toMail, string dislpayName, string subject, string emailBody)
        {
            List<string> toMails = new List<string>();
            toMails.Add(toMail);

            return CreateMailMessage(toMails, dislpayName, subject, emailBody);
        }
